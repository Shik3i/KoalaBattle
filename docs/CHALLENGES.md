...
Draft is a persistent solo campaign above the normal match engine. The registry contains
regional Gym/Trial, Elite Four and Champion routes for Generations I–IX plus an **All Generations**
route. Quick Start chooses one regional route for the run; Custom Draft can choose the route
explicitly. Every stage creates a normal immutable KoalaBattle match and replay.
...
After a completed route the result page derives a per-Pokémon performance recap from the saved
events: battles and switch-ins, active turns, moves, HP-equivalent damage dealt/taken, healing,
knockouts, faints, critical hits and statuses inflicted. The same finalized roster can then be
continued into any other registered route or All Generations without drafting or revalidation.
...
2. Open `/challenges` for Quick Start (a seeded random regional route), or `/challenges/new` and choose who battles, the difficulty, and whether to Quick Sim,
