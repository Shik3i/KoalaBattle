...
<style>
  .caption-safe{position:absolute;z-index:24;left:10%;right:10%;bottom:14%;display:flex;justify-content:center;pointer-events:none}.caption-safe span{max-width:70rem;padding:.45rem .8rem;border-radius:.45rem;background:rgba(0,0,0,.82);box-shadow:0 2px 14px rgba(0,0,0,.45);color:white;font:800 clamp(1rem,2.2vw,2rem)/1.25 system-ui;text-align:center;text-wrap:balance}.caption-safe.vertical{left:7%;right:7%;bottom:35%}.caption-safe.vertical span{font-size:clamp(1.1rem,4.4vw,2.2rem);max-width:28rem}@media(prefers-reduced-motion:reduce){.caption-safe{transition:none}}
</style>
