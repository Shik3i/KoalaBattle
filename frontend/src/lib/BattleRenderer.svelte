<script lang="ts">
  import { formatTier } from '$lib/format-label';
  ...
  // Showdown ids have no separators, so only a vocabulary can split them. This used to
  // special-case `randombattle` and otherwise hand the whole run to toUpperCase, which put
  // GEN 9 · KOALABATTLECANONICALNATDEXDRAFT on the intro card of every draft battle.
  function formatName(id: string, gen: number) {
    if (!id) return `GEN ${gen}`;
    return `GEN ${gen} · ${(formatTier(id) || 'custom game').toUpperCase()}`;
  }
