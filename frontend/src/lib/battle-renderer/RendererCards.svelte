...
<style>
  ...
  .winner-format{color:var(--r-dim);font:600 calc(var(--hud-scale,1) * clamp(.6rem,.78cqw,.82rem)) var(--mono);letter-spacing:.12em;text-transform:uppercase}
  ...
  .recap header span{color:var(--r-dim);font:600 calc(var(--hud-scale,1) * clamp(.58rem,.74cqw,.78rem)) var(--mono)}
  ...
  .recap li em{color:var(--r-dim);font:600 calc(var(--hud-scale,1) * clamp(.56rem,.72cqw,.76rem)) var(--mono);font-style:normal}
  .recap li u{padding:.04rem .3rem;border-radius:4px;background:color-mix(in srgb,var(--champion) 26%,transparent);color:#fff;font:800 calc(var(--hud-scale,1) * clamp(.52rem,.68cqw,.72rem)) var(--mono);text-decoration:none}
  ...
  .versus{display:grid;grid-template-columns:1fr auto 1fr;align-items:center;gap:clamp(.6rem,2.4cqw,2.4rem);width:min(94%,58cqw)}
  ...
  .versus-side>i{color:rgba(240,246,242,.72);font:700 calc(var(--hud-scale,1) * clamp(.6rem,.9cqw,.95rem)) var(--mono);font-style:normal;text-transform:uppercase}
