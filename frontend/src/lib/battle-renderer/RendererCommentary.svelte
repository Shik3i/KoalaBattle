...
<style>
  ...
  .battle-action-feed p{color:#c1d1c7;font:650 calc(var(--hud-scale,1) * clamp(.58rem,.72cqw,.76rem))/1.35 var(--display)}
  ...
  :global(.battle-renderer[data-layout='standard-vertical']) .battle-action-feed{top:36%;right:4%;left:4%;width:auto;max-height:31%}
  @container(max-width:760px){.battle-action-feed{top:36%;right:3%;left:3%;width:auto;max-height:31%;padding:.48rem .58rem}.battle-action-feed strong{font-size:calc(var(--hud-scale,1) * .76rem)}.battle-action-feed p{font-size:calc(var(--hud-scale,1) * .65rem)}.action-feed-turn:not(.current) article:not(:first-of-type){display:none}}
