...
{#if snapshot}
  <section class="transport panel" aria-label="Replay controls">
    <header class="transport-head"><div><span class="eyebrow">Interactive replay</span><h2>Watch in your browser</h2></div><p>Play continuously or inspect the battle event by event and turn by turn.</p></header>
    <div class="transport-buttons">
      ...
      <div><button on:click={() => timeline?.previousEvent()} disabled={snapshot.index === 0}>Previous event</button><button class="play" on:click={togglePlayback}><i class={`ph ${snapshot.playing ? 'ph-pause' : 'ph-play'}`} aria-hidden="true"></i>{snapshot.playing ? 'Pause' : 'Play'}</button><button on:click={() => timeline?.nextEvent()} disabled={snapshot.index >= snapshot.eventCount}>Next event</button></div>
    ...
    <div class="render-settings">
      <label>Speed<select value={config.playbackSpeed} on:change={(event) => updateConfig({ playbackSpeed: speedFrom(event.currentTarget.value) })}><option value={0.5}>0.5×</option><option value={1}>1×</option><option value={2}>2×</option><option value={4}>4×</option><option value="instant">Max</option></select></label>
...
<style>
  .challenge-back{display:inline-flex;align-items:center;min-height:24px;gap:.35rem;margin-top:.35rem;color:var(--accent);font-size:.72rem}
  ...
  .productions small{display:block;color:var(--muted);font:0.72rem var(--mono)}
  ...
  .replay-head{display:flex;justify-content:space-between;align-items:end;gap:1rem;margin-bottom:1.5rem}.replay-head h1{margin:.3rem 0 0;font-size:clamp(1.7rem,4vw,3rem)}.replay-head>span{color:var(--muted);font:.72rem var(--mono);white-space:nowrap}.transport{display:grid;gap:1rem;margin-top:1rem;padding:1rem;border-color:color-mix(in srgb,var(--accent) 35%,var(--border));box-shadow:none}.transport-head{display:flex;align-items:end;justify-content:space-between;gap:1rem}.transport-head h2{margin:.2rem 0 0}.transport-head p{max-width:520px;margin:0;color:var(--muted);font-size:.78rem}.transport-buttons{display:grid;grid-template-columns:1fr auto 1fr;gap:.7rem}.transport-buttons>div{display:flex;gap:.4rem}.transport-buttons>div:last-child{justify-content:flex-end}.transport button{min-height:40px;padding:.55rem .8rem;border:1px solid var(--border);border-radius:.55rem;background:var(--panel-strong);color:var(--text);cursor:pointer}.transport button:disabled{opacity:.4;cursor:not-allowed}.transport .play{display:inline-flex;align-items:center;justify-content:center;gap:.35rem;min-width:88px;border-color:var(--accent);background:var(--accent);color:var(--accent-ink);font-weight:800}.timeline{display:grid;grid-template-columns:auto 1fr auto;align-items:center;gap:.8rem}.timeline input{min-height:20px;padding:0;accent-color:var(--accent)}.timeline output{min-width:3.5rem;text-align:right;font:0.72rem var(--mono)}.render-settings{display:grid;grid-template-columns:repeat(4,minmax(130px,1fr));gap:.7rem;padding-top:1rem;border-top:1px solid var(--border)}.render-settings select{min-height:40px;padding:.5rem}.error{margin-top:1rem}@media(max-width:850px){.transport-head{align-items:start;flex-direction:column}.transport-buttons{grid-template-columns:1fr}.transport-buttons>div,.transport-buttons>div:last-child{justify-content:center}.render-settings{grid-template-columns:repeat(2,1fr)}}@media(max-width:560px){.replay-head{align-items:start;flex-direction:column}.transport-buttons>div{display:grid;grid-template-columns:1fr 1fr}.transport-buttons>div:nth-child(2){grid-template-columns:1fr}.render-settings{grid-template-columns:1fr}}
</style>
