<script lang="ts">
  ...
  async function saveProvider(provider: ProviderStatus) {
    ...
    try {
      ...
      saveProviderSetting(provider.id, { apiKey: '', baseUrl: draft.baseUrl });
      providerDrafts = {
        ...providerDrafts,
        [provider.id]: { apiKey: '', baseUrl: draft.baseUrl }
      };
  ...
  async function clearProvider(provider: ProviderStatus) {
    ...
    error = '';
    try {
      await configureProvider(provider.id, null, null, true);
      saveProviderSetting(provider.id, { apiKey: '', baseUrl: '' });
      providerDrafts = {
        ...providerDrafts,
...
<div class="settings-grid">
  <section class="panel providers"><span class="section-number"><i class="ph ph-plugs-connected" aria-hidden="true"></i> Providers</span><h2>Provider settings</h2><p>API keys are saved server-side in the local gitignored <code>.env</code> file and applied immediately. They survive backend and Docker rebuilds, never enter browser storage or match data, and are never returned by the API.</p><div class="provider-settings">{#each providers as provider}<article class:configured={provider.configured}><header><div><strong>{provider.label || providerLabel(provider.id)}</strong><small>{provider.configured ? `Configured · ${provider.source}` : provider.requires_api_key ? `Needs ${provider.environment_variable || 'API key'}` : 'Ready for a custom endpoint'}</small></div><span class:ready={provider.configured}>{provider.configured ? 'Ready' : 'Not configured'}</span></header><small>{provider.capabilities.structured_output ? 'Structured output' : 'Plain JSON'} · {provider.capabilities.model_listing ? 'model discovery' : 'custom model ID'} · default model <code>{provider.default_model}</code></small>{#if provider.known_models.length}<small>Selectable models · {#each provider.known_models as model}<code>{model}</code>{/each}</small>{/if}{#if provider.requires_api_key}<label>API key<input type="password" autocomplete="new-password" placeholder={provider.configured ? 'Saved in .env · enter a replacement only' : `Paste ${provider.label} key`} bind:value={providerDrafts[provider.id].apiKey} /></label>{/if}{#if provider.id === 'openai-compatible'}<label>Base URL<input type="url" placeholder="http://host.docker.internal:1234/v1" bind:value={providerDrafts[provider.id].baseUrl} /></label>{/if}<div class="provider-actions"><button class="button" type="button" on:click={() => saveProvider(provider)} disabled={savingProvider === provider.id}>{savingProvider === provider.id ? 'Saving…' : 'Save provider'}</button>{#if provider.configured || providerDrafts[provider.id]?.apiKey || providerDrafts[provider.id]?.baseUrl}<button class="button secondary" type="button" on:click={() => clearProvider(provider)} disabled={savingProvider === provider.id}>Clear saved setting</button>{/if}</div></article>{/each}</div><a class="preview-link" href="/new"><i class="ph ph-sword" aria-hidden="true"></i>Choose a configured provider for a battle</a></section>
  ...
  <section class="panel assets"><header><div><span class="section-number">03</span><h2>Local assets</h2></div><button class="button secondary" on:click={rescan}>Rescan assets</button></header><p>Optional user-installed media under <code>{assets?.root || 'data/assets'}</code>. KoalaBattle never downloads or bundles Pokémon media.</p>{#if assets && !assets.pokemon_species}
    <!-- A fresh checkout has no sprites by design. This is where the count says zero,
         so it is also where the way to fix it belongs. -->
    <div class="asset-hint" role="note">
      <i class="ph ph-download-simple" aria-hidden="true"></i>
      <div>
        <strong>No Pokémon sprites installed</strong>
        <small>Battles render labelled placeholders until you install them. The opt-in downloader fetches them from the official Pokémon Showdown deployment:</small>
        <code>python3 scripts/setup_assets.py install</code>
        <small>Add <code>--profile full</code> for animated sprites. These files stay local, are never committed, and are not covered by KoalaBattle's MIT license — see <code>docs/ASSETS.md</code>.</small>
      </div>
    </div>
  {/if}
...
<style>
  .settings-grid{display:grid;grid-template-columns:repeat(2,1fr);gap:1rem}.panel{position:relative;padding:1.5rem;box-shadow:none}.panel p{color:var(--muted);line-height:1.6}.section-number{color:var(--accent);font:0.72rem var(--mono)}.panel h2{margin:.4rem 0}.providers,.personas{grid-column:1/-1}.provider-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:.6rem}.provider-cards article{display:grid;gap:.3rem;padding:1rem;border:1px solid var(--border);border-radius:.7rem;background:var(--panel-strong)}.provider-cards article>span{color:var(--muted);font:0.72rem var(--mono);text-transform:uppercase}.provider-cards article>span.ready{color:var(--accent)}.provider-cards strong{text-transform:capitalize}.provider-cards small{color:var(--muted);font:0.72rem var(--mono)}.persona-cards{display:grid;grid-template-columns:repeat(3,1fr);gap:.7rem}.persona-cards article{display:grid;gap:.55rem;padding:1rem;border:1px solid var(--border);border-radius:.7rem;background:var(--panel-strong)}.persona-cards header{display:flex;align-items:start;justify-content:space-between;gap:.7rem}.persona-cards header div{display:grid;gap:.25rem}.persona-cards header small,.persona-mode{color:var(--muted);font:0.72rem var(--mono);text-transform:uppercase}.persona-cards article>small{color:var(--muted);font:0.72rem var(--mono);line-height:1.45}.persona-cards article>small.ready{color:var(--accent)}.persona-cards details{margin:0}.persona-cards details code{max-height:140px;overflow:auto}.file-input{display:grid;gap:.3rem;font-size:.72rem}.file-input input{font:0.72rem var(--mono)}.theme-options{display:flex;gap:.7rem;margin-top:1.2rem}.theme-options button{flex:1;min-height:70px;border:1px solid var(--border);border-radius:.7rem;background:var(--panel-strong);color:var(--text);cursor:pointer}.theme-options button.active{border-color:var(--accent);box-shadow:inset 0 0 0 2px var(--accent)}.form-grid{display:grid;grid-template-columns:1fr 1fr;gap:.7rem}.toggles{display:flex;flex-wrap:wrap;gap:.8rem;margin-top:1rem}.toggles label{display:flex;align-items:center;gap:.45rem}.toggles input{width:18px;min-height:18px}.assets,.obs{grid-column:1/-1}.assets header{display:flex;justify-content:space-between;align-items:center;gap:1rem}.asset-hint{display:grid;grid-template-columns:auto minmax(0,1fr);gap:.75rem;margin:.7rem 0;padding:.85rem 1rem;border:1px solid color-mix(in srgb,var(--accent) 45%,var(--border));border-radius:.7rem;background:color-mix(in srgb,var(--accent) 7%,var(--panel))}.asset-hint>i{color:var(--accent);font-size:1.35rem}.asset-hint>div{display:grid;gap:.3rem;min-width:0}.asset-hint code{overflow-x:auto;padding:.4rem .55rem;border-radius:.4rem;background:var(--surface);font-size:0.72rem}.asset-summary{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;margin:1.2rem 0;overflow:hidden;border:1px solid var(--border);border-radius:.7rem;background:var(--border)}.asset-summary>div{display:grid;padding:1rem;background:var(--panel-strong)}.asset-summary strong{font-size:1.6rem}.asset-summary strong.ok{color:var(--accent)}.asset-summary span{color:var(--muted);font:0.72rem var(--mono)}.asset-categories{display:grid;grid-template-columns:repeat(2,1fr);gap:.5rem}.asset-categories>div{display:grid;grid-template-columns:24px 1fr;align-items:center;padding:.7rem;border:1px solid var(--border);border-radius:.55rem}.asset-categories span{grid-row:1/3;color:var(--muted)}.asset-categories span.installed{color:var(--accent)}.asset-categories strong{font-size:.75rem;text-transform:capitalize}.asset-categories small{color:var(--muted);font:0.72rem var(--mono)}details{margin-top:.8rem;padding:.8rem;border:1px solid var(--border);border-radius:.55rem}details code{display:block;margin-top:.6rem;overflow-wrap:anywhere}.asset-debug{display:grid;grid-template-columns:1fr auto;align-items:end;gap:.7rem;margin-top:1rem}.asset-debug code{grid-column:1/-1;padding:.8rem;border-radius:.5rem;background:var(--panel-strong);overflow-wrap:anywhere}.obs-spec{display:grid;grid-template-columns:repeat(3,1fr);gap:1px;margin:1rem 0;background:var(--border)}.obs-spec span{display:grid;padding:1rem;background:var(--panel-strong);color:var(--muted);font:0.72rem var(--mono)}.obs-spec strong{color:var(--text);font:700 1rem var(--display)}.copy-row{display:grid;grid-template-columns:1fr auto;gap:.6rem}.copy-row input{font:0.72rem var(--mono)}.preview-link{display:inline-flex;align-items:center;min-height:24px;margin-top:1rem;color:var(--accent);font-weight:700}.connection{grid-column:1/-1}dl{display:grid;grid-template-columns:auto 1fr;gap:.7rem}dt{color:var(--muted)}dd{margin:0;overflow-wrap:anywhere;font-family:var(--mono);font-size:.8rem}code{font-family:var(--mono);color:var(--accent)}@media(max-width:760px){.settings-grid,.form-grid,.asset-categories,.provider-cards,.persona-cards{grid-template-columns:1fr}.providers,.personas,.assets,.obs,.connection{grid-column:auto}.asset-summary,.obs-spec{grid-template-columns:1fr}.asset-debug{grid-template-columns:1fr}.copy-row{grid-template-columns:1fr}.assets header{align-items:start;flex-direction:column}}
  .section-number,.preview-link,.theme-options button{display:flex;align-items:center;gap:.4rem}.theme-options button{justify-content:center;transition:transform .16s ease,border-color .16s ease,box-shadow .16s ease}.theme-options button:hover{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 45%,var(--border))}.provider-cards article{transition:transform .18s ease,border-color .18s ease}.provider-cards article:hover{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 30%,var(--border))}.provider-cards strong{text-transform:none}.provider-settings{display:grid;grid-template-columns:repeat(2,1fr);gap:.7rem}.provider-settings article{display:grid;gap:.6rem;padding:1rem;border:1px solid var(--border);border-radius:.7rem;background:var(--panel-strong)}.provider-settings article.configured{border-color:color-mix(in srgb,var(--accent) 50%,var(--border))}.provider-settings header{display:flex;justify-content:space-between;align-items:start;gap:.7rem}.provider-settings header div{display:grid;gap:.25rem}.provider-settings header span{color:var(--muted);font:0.72rem var(--mono);text-transform:uppercase}.provider-settings header span.ready{color:var(--accent)}.provider-settings small{color:var(--muted);font:0.72rem var(--mono);line-height:1.45}.provider-settings label{display:grid;gap:.3rem;font-size:.75rem}.provider-settings input{width:100%}.provider-actions{display:flex;flex-wrap:wrap;gap:.5rem}
</style>
