...
<div class="settings-grid">
  ...
  <section class="panel providers"><span class="section-number">Speech</span><h2>Audio providers & VoicePresets</h2><p>Edge Neural is the free natural default and sends public commentary to Microsoft's online speech service while production audio is prepared. Basic offline system voices remain available. Paid providers require separate configuration and approval.</p><div class="provider-cards">{#each speechProviders as provider}<article><span class:ready={provider.available}>{provider.available ? 'Available' : 'Unavailable'}</span><strong>{provider.id}</strong><small>{provider.paid ? 'paid / explicit action' : 'zero-cost'} · {provider.supports_timestamps ? 'timestamps' : 'normalized caption timing'}</small><small>{provider.detail}</small></article>{/each}</div><p>{voices.length} persisted VoicePresets · generated media: <code>data/audio/</code></p></section>
