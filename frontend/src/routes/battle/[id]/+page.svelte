<script lang="ts">
  ...
  onMount(() => {
    ...
    return () => { socket?.close(); timeline?.destroy(); };
  });
  ...
  async function submit(side: Side) {
    const request = pending[side]; if (!request) return; error = '';
    try {
    ...
    {#each Object.entries(pending) as [sideValue, request]}
      ...
      {#if request}
        <article class="manual panel">
          <header><div><span class="eyebrow">{side} · Turn {request.turn} · {request.prompt_template_version}</span><h2>{side.toUpperCase()} manual workspace</h2></div><button class="button secondary" on:click={() => copyPrompt(side)}>{copied === side ? 'Copied' : 'Copy prompt'}</button></header>
    ...
    {#each [...match.decisions].reverse() as record}
      <details class="decision panel">
        ...
        <div class="decision-grid">
          ...
          {#if record.generated_prompt}<label>Prompt<textarea readonly value={record.generated_prompt}></textarea></label>{/if}
          {#if record.raw_response}<label>Raw provider response<textarea readonly value={record.raw_response}></textarea></label>{/if}
        </div>
      </details>
    {/each}
  </div>
