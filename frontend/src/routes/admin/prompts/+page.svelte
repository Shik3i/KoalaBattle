...
<style>
  .page-head{display:flex;align-items:end;justify-content:space-between;gap:2rem}.page-head h1{margin:.3rem 0;font-size:clamp(2rem,6vw,4rem)}.page-head p{max-width:700px;color:var(--muted)}.controls{display:grid;grid-template-columns:1.4fr 1.2fr 1fr 1fr auto;align-items:end;gap:.75rem;margin-top:2rem;padding:1rem;box-shadow:none}.output{display:grid;gap:1rem;margin-top:1rem}.metrics{display:flex;gap:1rem;flex-wrap:wrap}.metrics span{display:grid;padding:.7rem 1rem;border:1px solid var(--border);border-radius:.7rem;color:var(--muted);font:0.72rem var(--mono)}.metrics strong{color:var(--text);font-size:1rem}.output textarea{min-height:580px;font:400 0.72rem/1.55 var(--mono)}.output details,.unavailable{padding:1rem;box-shadow:none}.output pre{max-height:500px;overflow:auto;white-space:pre-wrap;overflow-wrap:anywhere;color:var(--muted);font:400 0.72rem/1.5 var(--mono)}@media(max-width:1000px){.controls{grid-template-columns:1fr 1fr}.controls button{grid-column:1/-1}}@media(max-width:600px){.page-head{align-items:start;flex-direction:column}.controls{grid-template-columns:1fr}.controls button{grid-column:auto}}
</style>
