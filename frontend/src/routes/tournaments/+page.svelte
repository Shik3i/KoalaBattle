...
<div class="page-head"><div><span class="eyebrow">Competition operations</span><h1>Tournaments</h1></div><a class="button" href="/tournaments/new">Create tournament</a></div>
  ...
  <div class="tournament-list">
    {#each tournaments as tournament}
      <a class="panel" href={`/tournaments/${tournament.id}/control`}>
        ...
        <span class={`status-pill ${tournament.status}`}>{tournament.status}</span>
      </a>
    {/each}
  </div>
{/if}
...
<style>
  .tournament-list{display:grid;gap:.7rem}.tournament-list>a{display:grid;grid-template-columns:1fr auto auto auto;align-items:center;gap:2rem;padding:1.2rem 1.4rem;box-shadow:none}.tournament-list h2{margin:.25rem 0 0}.progress{display:grid;text-align:right}.progress strong{font-size:1.1rem}.progress span{color:var(--muted);font:.6rem var(--mono)}.empty{padding:4rem;text-align:center}.empty p{color:var(--muted)}@media(max-width:760px){.page-head{align-items:stretch;flex-direction:column}.tournament-list>a{grid-template-columns:1fr 1fr;gap:1rem}.tournament-list>a>div:first-child{grid-column:1/-1}.progress{text-align:left}}
</style>
