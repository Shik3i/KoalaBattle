...
<details class="run-details panel"><summary>Saved run details</summary><dl><div><dt>Seed</dt><dd>{run.seed}</dd></div><div><dt>Definition</dt><dd>{run.definition.id} · v{run.definition.version}</dd></div><div><dt>Format</dt><dd>{run.definition.format}</dd></div><div><dt>Pricing</dt><dd>{run.pricing.board_name} · {run.pricing.context}</dd></div><div><dt>Catalog</dt><dd><code>{run.pricing.catalog_hash}</code></dd></div><div><dt>Imported</dt><dd>{new Date(run.pricing.imported_at).toLocaleString()}</dd></div></dl></details>
