  ...
  function setView(nextView: ChallengeRunView) {
    ...
    if (nextFingerprint && (firstRoll || (priorFingerprint && priorFingerprint !== nextFingerprint))) {
      ...
      if (mode) {
        ...
        if (rollRevealTimer) clearTimeout(rollRevealTimer);
        rollRevealTimer = setTimeout(() => (rollReveal = null), DRAFT_ROLL_DURATION_MS + 60);
      }
    }
...
{#if run.status === 'drafting' && run.current_offer}
  <section id="draft" class="draft panel" aria-labelledby="draft-title">
    ...
    <header class="roll-result">
      <div class="roll-copy"><span class="eyebrow"><i class="ph ph-sparkle" aria-hidden="true"></i> Draft roll · Pick {run.current_offer.round} of {run.definition.draft_rules.roster_size}</span><h2 id="draft-title" class="draft-reels" aria-live="polite"><span class:spinning={Boolean(rollReveal && rollReveal.mode !== 'type')} class:locked={rollReveal?.mode === 'type'} class="draft-reel generation-reel"><small>GEN</small><span class="reel-window"><span class="reel-track" style={`--reel-offset:${-((rollReveal?.generations.length || 1) - 1) * 42}px`}>{#each rollReveal?.generations || [run.current_offer.generation] as generation}<b>{generationRomanNumeral(generation)}</b>{/each}</span></span></span><i class="ph ph-x" aria-hidden="true"></i><span class:spinning={Boolean(rollReveal && rollReveal.mode !== 'generation')} class:locked={rollReveal?.mode === 'generation'} class="draft-reel type-reel"><span class="reel-window"><span class="reel-track" style={`--reel-offset:${-((rollReveal?.types.length || 1) - 1) * 42}px`}>{#each rollReveal?.types || [run.current_offer.type] as type}<b style={`--type-color:${pokemonTypeColor(type)}`}>{type}</b>{/each}</span></span></span></h2><p>{rollReveal ? (rollReveal.mode === 'both' ? 'Rolling Generation and Type…' : rollReveal.mode === 'type' ? 'Generation locked · rolling Type…' : 'Type locked · rolling Generation…') : 'Choose one Pokémon. Your next Generation and Type roll automatically.'}</p></div>
...
<style>
  ...
  @media(max-width:600px){.stage-focus{align-items:stretch;flex-direction:column}.auto-control{justify-items:start;text-align:left}.battle-summary{grid-template-columns:1fr}}
