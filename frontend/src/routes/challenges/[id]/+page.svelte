<script lang="ts">
  ...
  import type { ChallengeRunView, DraftCandidate, EvSpread, PricingStatus } from '$lib/types';
...
{#if run.picks.length}
  <section class="roster panel"><header><div><span class="eyebrow">Drafted roster</span><h2>{run.picks.length} of {run.definition.draft_rules.roster_size} locked</h2></div><strong>{run.credits_remaining} credits left</strong></header><div>{#each run.picks as pick}<article><PokemonSprite species={pick.candidate.species} size="medium" decorative /><span>Round {pick.round} · {pick.selected_by === 'human' ? 'Me' : pick.selected_by === 'agent' ? 'AI' : 'Random'}</span><h3>{pick.candidate.species}</h3><p class="type-badges"><TypeBadges types={pick.candidate.types} compact /></p><small>{pick.candidate.base_stat_total ? `BST ${pick.candidate.base_stat_total}` : `Gen ${pick.candidate.introduction_generation}`}</small><strong>{pick.candidate.points}<small> cr</small></strong></article>{/each}</div></section>
{/if}
...
<style>
  ...
  @media(max-width:1100px){.ev-cards{grid-template-columns:1fr}.roster>div{grid-template-columns:repeat(3,1fr)}}
