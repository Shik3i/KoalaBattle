<script lang="ts">
  ...
  let initializedRun = '';
  let scaffoldRun = '';
  type RerollKind = 'all' | 'type' | 'generation';
  type ConfirmationAction = 'reroll-all' | 'reroll-type' | 'reroll-generation' | 'finalize' | 'cancel' | 'reset-all' | 'restore';
  type Confirmation = {
  ...
  function rerollsRemaining(count: number, label: string) {
    return `${count} ${label}${count === 1 ? '' : 's'} will remain.`;
  }
  function requestReroll(kind: RerollKind) {
    if (!run?.current_offer) return;
    const copy = {
      all: {
        title: 'Roll everything again?',
        detail: `Generation, Type, and all three Pokémon change. The current cards leave this run permanently; ${rerollsRemaining(run.rerolls_remaining - 1, 'full reroll')}`,
        label: 'Reroll both'
      },
      type: {
        title: 'Roll a different Type?',
        detail: `Generation ${run.current_offer.generation} stays. The Type and all three Pokémon change; ${rerollsRemaining(run.type_rerolls_remaining - 1, 'Type reroll')}`,
        label: 'Reroll Type'
      },
      generation: {
        title: 'Roll a different Generation?',
        detail: `${run.current_offer.type} stays. The Generation and all three Pokémon change; ${rerollsRemaining(run.generation_rerolls_remaining - 1, 'Generation reroll')}`,
        label: 'Reroll Generation'
      }
    }[kind];
    openConfirmation({
      action: `reroll-${kind}`,
  ...
  function requestFinalizeTeam() {
    ...
    openConfirmation({
      ...
      detail: 'Showdown validates the exact sets first. After validation, drafted species, abilities, and Training Camp EVs stay fixed for the campaign.',
  ...
  function historyOutcomeLabel(outcome: ChallengeRunView['run']['draft_history'][number]['outcome']) {
    ...
    return 'Generation + Type rerolled';
  }
  ...
  async function acceptConfirmation() {
    ...
    if (action.startsWith('reroll-') && run.current_offer) {
      const kind = action.slice('reroll-'.length) as RerollKind;
      ...
      trainingNotice = 'All Training Camp allocations reset.';
...
{#if !['drafting','training','team_review'].includes(run.status)}<section id="campaign" class="campaign panel">
  <header><div><span class="eyebrow">Campaign route</span><h2>{view.statistics.stages_cleared} of {view.stages.length} stages cleared</h2></div><div class="campaign-stats"><span><strong>{view.statistics.wins}-{view.statistics.losses}-{view.statistics.draws}</strong>record</span><span><strong>{view.statistics.total_battles}</strong>battles</span><span><strong>{view.statistics.total_turns}</strong>turns</span><span><strong>{run.picks.length}</strong>drafted</span></div></header>
...
{#if run.status === 'drafting' && run.current_offer}
  <section id="draft" class="draft panel" aria-labelledby="draft-title">
    ...
    <header class="roll-result">
      ...
      <div class="reroll-wallet" aria-label="Rerolls remaining"><span title="Reroll Generation, Type, and all cards"><i class="ph ph-arrows-clockwise" aria-hidden="true"></i><strong>{run.rerolls_remaining}</strong><b>Both</b></span><span title="Keep the Generation and reroll Type plus all cards"><i class="ph ph-palette" aria-hidden="true"></i><strong>{run.type_rerolls_remaining}</strong><b>Type</b></span><span title="Keep the Type and reroll Generation plus all cards"><i class="ph ph-clock-counter-clockwise" aria-hidden="true"></i><strong>{run.generation_rerolls_remaining}</strong><b>Gen</b></span></div>
...
{#if ['training','team_review'].includes(run.status)}
  <section id="training" class="training panel" aria-labelledby="training-title"><header><div><span class="eyebrow">Roster complete · Training Camp</span><h2 id="training-title">Recommended EVs applied</h2><p>Every drafted Pokémon starts with its own legal recommended spread. Use a preset or fine-tune only if you want to.</p></div><div class="training-actions"><button class="button ghost compact" on:click={requestResetAll}>Reset all</button><div class="ev-counter"><strong>{evUsed}</strong><span>team EV total</span></div></div></header>
