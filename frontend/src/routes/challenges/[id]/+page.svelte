<script lang="ts">
  ...
  let rollReveal: { generation: number; type: string } | null = null;
  ...
  onMount(() => {
    void refresh();
    timer = setInterval(() => { if (view?.run.active_match_id) void refresh(false); }, 1000);
  ...
  function setView(nextView: ChallengeRunView) {
    ...
    if (priorFingerprint && nextFingerprint && priorFingerprint !== nextFingerprint) {
      rollReveal = { generation: nextRun.current_offer!.generation, type: nextRun.current_offer!.type };
      if (rollRevealTimer) clearTimeout(rollRevealTimer);
      rollRevealTimer = setTimeout(() => (rollReveal = null), 760);
    }
...
<section class="continue-card panel" aria-labelledby="continue-title"><div><span class="eyebrow">Continue where you left off</span><h2 id="continue-title">{primaryLabel(run)}</h2><p>{run.active_match_id ? (run.battle_experience === 'quick-sim' ? 'The real stage match is resolving in the background. Result and replay appear below automatically.' : 'The stage match is already linked and running. Your pending Human turn is restored on the battle screen.') : `Saved revision ${run.revision} · updated ${new Date(run.updated_at).toLocaleString()}`}</p></div><a class="button" href={primaryHref(run)}>{primaryLabel(run)}<i class="ph ph-arrow-right" aria-hidden="true"></i></a></section>
...
{#if run.status === 'drafting' && run.current_offer}
  <section id="draft" class="draft panel" aria-labelledby="draft-title">
    {#if rollReveal}<div class="roll-reveal" role="status" aria-live="polite"><span>Next draft roll</span><strong>Generation {rollReveal.generation}</strong><i class="ph ph-x" aria-hidden="true"></i><strong>{rollReveal.type}</strong></div>{/if}
...
<style>
  .advanced-team-link{display:flex;align-items:center;justify-content:flex-end;gap:.7rem;margin:-.35rem 0 .9rem;color:var(--muted);font:.6rem var(--mono)}.quick-sim-spinner{display:flex;align-items:center;gap:.4rem;color:var(--accent);font:700 .68rem var(--mono)}.quick-sim-spinner i{animation:spin .8s linear infinite}@media(prefers-reduced-motion:reduce){.quick-sim-spinner i{animation:none}}
  .roll-reveal{position:absolute;z-index:20;inset:0;display:flex;align-items:center;justify-content:center;gap:.8rem;border-radius:inherit;background:color-mix(in srgb,var(--panel-strong) 92%,transparent);backdrop-filter:blur(12px);animation:round-transition .76s ease both}.roll-reveal span{position:absolute;transform:translateY(-2.2rem);color:var(--muted);font:.58rem var(--mono);letter-spacing:.14em;text-transform:uppercase}.roll-reveal strong{font-size:clamp(1.4rem,4vw,2.8rem)}.roll-reveal i{color:var(--accent)}@keyframes round-transition{0%{opacity:0}18%,72%{opacity:1}100%{opacity:0}}@media(prefers-reduced-motion:reduce){.roll-reveal{animation:none;opacity:.96}}
