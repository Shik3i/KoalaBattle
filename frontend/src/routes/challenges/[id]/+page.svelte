<script lang="ts">
  ...
  function rerollBlockedReason(kind: RerollKind): string {
    if (!run || !view) return '';
    const remaining = kind === 'pokemon' ? run.rerolls_remaining
      : kind === 'type' ? run.type_rerolls_remaining
      : run.generation_rerolls_remaining;
    if (!remaining) return 'Already used this run.';
    const possible = kind === 'pokemon' ? view.can_reroll
      : kind === 'type' ? view.can_reroll_type
      : view.can_reroll_generation;
    if (!possible) {
      return kind === 'pokemon'
...
{:else if ['ready','battle_queued','battling','stage_result','preparing'].includes(run.status) && view.current_stage}
  ...
  <section id="current-stage" class="stage-hero panel" style={`--stage-accent:${view.current_stage.visual_accent}`} aria-label={`Current battle ${run.current_stage_index + 1} of ${view.stages.length}`}>
    ...
    <div class="stage-action">
      {#if run.status === 'preparing' || run.active_match_id || run.auto_run_paused || autoCountdown !== null}<strong role="status">{run.status === 'preparing' ? 'Preparing your team' : run.active_match_id ? (run.battle_experience === 'quick-sim' ? 'Simulating real Showdown battle…' : 'Battle in progress') : run.auto_run_paused ? 'Auto-Run paused' : `Next battle in ${autoCountdown}…`}</strong>{/if}
      <div class="stage-buttons">
        ...
        {:else if run.active_match_id}
          {#if run.battle_experience !== 'quick-sim'}<a class="button" href={`/battle/${run.active_match_id}`}>Resume battle<i class="ph ph-arrow-up-right" aria-hidden="true"></i></a>{:else}<span class="quick-sim-spinner" role="status"><i class="ph ph-circle-notch" aria-hidden="true"></i>Simulating</span>{/if}
          {#if autoRunAvailable && !run.auto_run_paused}<button class="button ghost compact" disabled={Boolean(loading)} on:click={pauseAutoRun}>{loading === 'pause-auto' ? 'Pausing…' : 'Pause Auto-Run'}</button>{/if}
        ...
        {:else if run.auto_advance_at && autoRunAvailable}
          ...
          <button class="button ghost compact" disabled={Boolean(loading)} on:click={pauseAutoRun}>{loading === 'pause-auto' ? 'Pausing…' : 'Pause Auto-Run'}</button>
        {:else}
...
{#if run.stage_results.length}
  <details id="run-archive" class="battle-history panel" open={run.status === 'completed'}>
    <summary>Battle history · {run.stage_results.length} recorded {run.stage_results.length === 1 ? 'battle' : 'battles'}</summary>
...
{#if run.picks.length && run.status !== 'drafting'}
  ...
  <section class="roster-strip" aria-label="Drafted roster">{#each run.picks as pick}<span title={`${pick.candidate.species} · ${pick.candidate.types.join('/')}`}><PokemonSprite species={pick.candidate.species} size="small" decorative /><b>{pick.candidate.species}</b></span>{/each}</section>
{/if}
...
<style>.battle-summary{display:grid;grid-template-columns:1fr 1fr;gap:.5rem;margin-top:.6rem}.battle-summary>div{padding:.5rem .55rem;border:1px solid var(--border);border-radius:.65rem;background:var(--panel-strong)}.battle-summary>div>strong,.battle-summary>div>small{display:block;margin-bottom:.4rem}.battle-summary>div>small{margin:.45rem 0 .25rem;color:var(--muted);font:.54rem var(--mono);text-transform:uppercase}.battle-summary>div>div{display:flex;flex-wrap:wrap;gap:.35rem}.battle-summary span{display:flex;align-items:center;gap:.28rem;padding:.18rem .38rem;border-radius:.45rem;background:var(--surface);font-size:.62rem}.battle-summary span.fainted{opacity:.68;filter:grayscale(.75)}.battle-summary span.opponent{border:1px solid color-mix(in srgb,var(--danger) 30%,var(--border))}.battle-summary em{color:var(--muted);font:.6rem var(--mono)}.next-countdown{display:flex;align-items:center;gap:.35rem;margin-top:.7rem!important;color:var(--accent)!important;font-weight:750}.result-copy{min-width:0;flex:1}
  ...
  @keyframes reel-lock{0%,74%{border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 8%,transparent)}88%{border-color:color-mix(in srgb,var(--accent) 90%,white);box-shadow:0 0 22px 2px color-mix(in srgb,var(--accent) 55%,transparent),0 0 0 3px color-mix(in srgb,var(--accent) 22%,transparent);transform:scale(1.06)}100%{border-color:var(--accent);box-shadow:0 0 0 2px color-mix(in srgb,var(--accent) 10%,transparent);transform:scale(1)}}
  .reroll-blocked{display:grid;gap:.2rem;margin:.5rem 0 0;padding:0;list-style:none}
  ...
  .run-error strong{display:block;font-size:.88rem}
  .run-error p{margin:.2rem 0!important;overflow-wrap:anywhere;font-size:.72rem}
