...
<form class="launch-card panel" on:submit|preventDefault={create}>
  ...
  <details class="advanced"><summary><i class="ph ph-sliders-horizontal" aria-hidden="true"></i>Advanced settings</summary><div class="advanced-grid"><label>Draft name<input bind:value={name} maxlength="120" required /></label><label>Draft seed<input type="number" bind:value={seed} required /></label><label>Cards per roll<input type="number" min="2" max="8" bind:value={choiceCount} /></label><div class="fixed-opponent"><span>Opponent controller</span><strong><i class="ph ph-lightning" aria-hidden="true"></i> Fast Auto</strong><small>Always local, tactical and switch-capable.</small></div></div>
