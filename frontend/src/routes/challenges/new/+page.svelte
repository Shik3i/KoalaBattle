...
{:else}
  ...
  <form class="wizard panel" on:submit|preventDefault={create}>
    {#if step === 0}
      <section aria-labelledby="campaign-step"><span class="eyebrow">Step 1 of 5</span><h2 id="campaign-step">Choose the campaign</h2><p>The first pack is a thirteen-stage Kanto route. Every run stores its exact rules and content version.</p><article class="campaign-card"><div class="campaign-mark"><i class="ph ph-map-trifold" aria-hidden="true"></i></div><div><strong>Kanto Gym Gauntlet</strong><span>Brock → eight Gyms → Elite Four → Champion</span><small>Gen 9 National Dex Draft singles · levels 50–100 · equal levels on both sides</small></div><span class="selected">Selected</span></article><div class="field-grid"><label>Challenge name<input bind:value={name} maxlength="120" required aria-describedby="name-help" /><small id="name-help">Shown in Challenge history and stage match titles.</small></label><label>Draft seed<input type="number" bind:value={seed} required aria-describedby="seed-help" /><small id="seed-help">The same seed, catalog, version, and rules reproduce draft offers.</small></label></div></section>
    {:else if step === 1}
