<script lang="ts">
  ...
  // Custom Draft keeps the established 3/1/1 defaults, but exposes all three
  // powers so advanced users can tune the run explicitly.
  let pokemonRerolls = 3;
  let typeRerolls = STANDARD_CHALLENGE_SETTINGS.typeRerolls;
  let generationRerolls = STANDARD_CHALLENGE_SETTINGS.generationRerolls;
  ...
  $: valid = Boolean(name.trim()) && Boolean(definitionId) && Number.isSafeInteger(Number(seed)) && choiceCount >= 2 && choiceCount <= 8
    && [pokemonRerolls, typeRerolls, generationRerolls].every((value) => Number.isInteger(Number(value)) && Number(value) >= 0 && Number(value) <= 20)
    && aiReady;
  ...
  async function create() {
    ...
    try {
      const view = await api<ChallengeRunView>('/api/challenges', { method: 'POST', body: JSON.stringify({
        ...
        draft_rules: { roster_size: 6, rerolls: Number(pokemonRerolls), type_rerolls: Number(typeRerolls), generation_rerolls: Number(generationRerolls), choice_count: Number(choiceCount), species_clause: true, draft_pool_mode: draftPoolMode }
      }) });
...
<div class="page-head"><div><span class="eyebrow">Draft</span><h1>{selectedDefinition?.name || 'Choose a campaign'}</h1><p>{selectedDefinition?.description || 'Draft six Pokémon, prepare them, then face real story teams from across the Pokémon regions.'}</p></div><a class="button ghost compact" href="/challenges">Draft history</a></div>
  ...
  <details class="advanced"><summary><i class="ph ph-sliders-horizontal" aria-hidden="true"></i>Advanced settings</summary><div class="advanced-grid"><label>Draft name<input bind:value={name} on:input={() => (nameEdited = true)} maxlength="120" required /></label><label>Draft seed<input type="number" bind:value={seed} required /></label><label>Cards per roll<input type="number" min="2" max="8" bind:value={choiceCount} /></label><label>Pokémon rerolls<input type="number" min="0" max="20" bind:value={pokemonRerolls} /><small>Replace only the offer Pokémon</small></label><label>Type rerolls<input type="number" min="0" max="20" bind:value={typeRerolls} /><small>Keep generation</small></label><label>Generation rerolls<input type="number" min="0" max="20" bind:value={generationRerolls} /><small>Keep type</small></label><div class="fixed-opponent"><span>Opponent controller</span><strong><i class="ph ph-lightning" aria-hidden="true"></i> Fast Auto</strong><small>Always local, tactical and switch-capable.</small></div></div>
    ...
    <p>Custom Draft reroll powers are configurable from 0–20. Quick Start uses 0 Pokémon rerolls plus 1 Type and 1 Generation reroll. Recommended legal EVs and abilities are applied automatically after the sixth pick.</p></details>
