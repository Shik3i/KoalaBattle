<script lang="ts">
  ...
  let rerolls = 3;
  let choiceCount = 3;
  let draftKind: 'human' | 'agent' = 'human';
  let battleType: 'human' | 'api' = 'human';
  let opponentType: 'random' | 'api' = 'random';
  ...
  $: needsAi = draftKind === 'agent' || battleType === 'api' || opponentType === 'api';
  $: aiReady = !needsAi || readyProviders.length > 0;
  $: valid = Boolean(name.trim()) && Number.isSafeInteger(Number(seed)) && rerolls >= 0 && rerolls <= 20 && choiceCount >= 2 && choiceCount <= 8 && aiReady;
  ...
  function chooseProvider(target: 'draft' | 'battle' | 'opponent', provider: ProviderKind) {
    const model = providers.find((item) => item.id === provider)?.default_model || '';
    if (target === 'draft') { draftProvider = provider; draftModel = model; }
  ...
  async function create() {
    ...
    try {
      const view = await api<ChallengeRunView>('/api/challenges', { method: 'POST', body: JSON.stringify({
        ...
        draft_rules: { roster_size: 6, rerolls: Number(rerolls), type_rerolls: 1, generation_rerolls: 1, choice_count: Number(choiceCount), species_clause: true }
      }) });
...
<div class="page-head"><div><span class="eyebrow">Draft Challenge</span><h1>Enter the Kanto Gym Gauntlet</h1><p>Draft six Pokémon, prepare them, then face Brock through Champion Blue.</p></div><a class="button ghost compact" href="/challenges">Challenge history</a></div>
...
<form class="launch-card panel" on:submit|preventDefault={create}>
  ...
  <div class="quick-choices">
    <fieldset><legend>Who drafts your team?</legend><div class="segmented"><button type="button" class:chosen={draftKind === 'human'} aria-pressed={draftKind === 'human'} on:click={() => (draftKind = 'human')}><i class="ph ph-user" aria-hidden="true"></i><span><strong>Me</strong><small>Pick every card</small></span></button><button type="button" class:chosen={draftKind === 'agent'} aria-pressed={draftKind === 'agent'} on:click={() => (draftKind = 'agent')}><i class="ph ph-robot" aria-hidden="true"></i><span><strong>AI</strong><small>Chooses each roll</small></span></button></div></fieldset>
    <fieldset><legend>Who battles the leaders?</legend><div class="segmented"><button type="button" class:chosen={battleType === 'human'} aria-pressed={battleType === 'human'} on:click={() => (battleType = 'human')}><i class="ph ph-game-controller" aria-hidden="true"></i><span><strong>Me</strong><small>Moves and switches</small></span></button><button type="button" class:chosen={battleType === 'api'} aria-pressed={battleType === 'api'} on:click={() => (battleType = 'api')}><i class="ph ph-robot" aria-hidden="true"></i><span><strong>AI</strong><small>Plays every stage</small></span></button></div></fieldset>
  </div>
  ...
  <details class="advanced"><summary><i class="ph ph-sliders-horizontal" aria-hidden="true"></i>Advanced settings</summary><div class="advanced-grid"><label>Challenge name<input bind:value={name} maxlength="120" required /></label><label>Draft seed<input type="number" bind:value={seed} required /></label><label>Full rerolls<input type="number" min="0" max="20" bind:value={rerolls} /></label><label>Cards per roll<input type="number" min="2" max="8" bind:value={choiceCount} /></label><label>Opponent controller<select bind:value={opponentType}><option value="random">Random baseline</option><option value="api">AI Agent</option></select></label></div>
    {#if draftKind === 'agent'}<div class="provider-row"><strong>Draft AI</strong><label>Provider<select value={draftProvider} on:change={(event) => chooseProvider('draft', event.currentTarget.value as ProviderKind)}>{#each readyProviders as item}<option value={item.id}>{item.label}</option>{/each}</select></label><label>Model{#if modelsFor(draftProvider).length}<select bind:value={draftModel}>{#each modelsFor(draftProvider) as model}<option value={model}>{draftProvider === 'deepseek' ? deepSeekModelLabel(model) : model}</option>{/each}</select>{:else}<input bind:value={draftModel} />{/if}</label></div>{/if}
...
<style>
  ...
  @media(max-width:650px){.quick-choices,.advanced-grid{grid-template-columns:1fr}.launch-card>header{grid-template-columns:auto 1fr}.selected{grid-column:1/-1;justify-self:start}.provider-row{grid-template-columns:1fr}.provider-row>strong{grid-column:auto}}
