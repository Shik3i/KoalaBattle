<script lang="ts">
  ...
  let battleExperience: 'quick-sim' | 'fast-watch' | 'normal' = 'quick-sim';
...
<form class="launch-card panel" on:submit|preventDefault={create}>
  ...
  <div class="quick-choices">
    <fieldset><legend>Who drafts?</legend><div class="segmented"><button type="button" class:chosen={draftKind === 'human'} aria-pressed={draftKind === 'human'} on:click={() => (draftKind = 'human')}><i class="ph ph-user" aria-hidden="true"></i><span><strong>Me</strong><small>Pick every card</small></span></button><button type="button" class:chosen={draftKind === 'agent'} aria-pressed={draftKind === 'agent'} on:click={() => (draftKind = 'agent')}><i class="ph ph-robot" aria-hidden="true"></i><span><strong>Auto / AI</strong><small>Chooses each roll</small></span></button></div></fieldset>
    <fieldset><legend>Who plays?</legend><div class="segmented three"><button type="button" class:chosen={battleType === 'tactical-auto'} aria-pressed={battleType === 'tactical-auto'} on:click={() => (battleType = 'tactical-auto')}><i class="ph ph-lightning" aria-hidden="true"></i><span><strong>Fast Auto</strong><small>Local · tactical · free</small></span></button><button type="button" class:chosen={battleType === 'human'} aria-pressed={battleType === 'human'} on:click={() => (battleType = 'human')}><i class="ph ph-game-controller" aria-hidden="true"></i><span><strong>Me</strong><small>Interactive</small></span></button><button type="button" class:chosen={battleType === 'api'} aria-pressed={battleType === 'api'} on:click={() => (battleType = 'api')}><i class="ph ph-brain" aria-hidden="true"></i><span><strong>LLM</strong><small>Uses a provider</small></span></button></div></fieldset>
    <fieldset class="experience"><legend>Battle experience</legend><div class="segmented three"><button type="button" class:chosen={battleExperience === 'quick-sim'} aria-pressed={battleExperience === 'quick-sim'} on:click={() => (battleExperience = 'quick-sim')}><i class="ph ph-fast-forward" aria-hidden="true"></i><span><strong>Quick Sim</strong><small>Result + replay</small></span></button><button type="button" class:chosen={battleExperience === 'fast-watch'} aria-pressed={battleExperience === 'fast-watch'} on:click={() => (battleExperience = 'fast-watch')}><i class="ph ph-monitor-play" aria-hidden="true"></i><span><strong>Fast Watch</strong><small>Watch at speed</small></span></button><button type="button" class:chosen={battleExperience === 'normal'} aria-pressed={battleExperience === 'normal'} on:click={() => (battleExperience = 'normal')}><i class="ph ph-play" aria-hidden="true"></i><span><strong>Normal</strong><small>Full pacing</small></span></button></div></fieldset>
  </div>
...
<style>
  ...
  @media(max-width:650px){.quick-choices,.advanced-grid{grid-template-columns:1fr}.segmented.three{grid-template-columns:1fr}.launch-card>header{grid-template-columns:auto 1fr}.selected{grid-column:1/-1;justify-self:start}.provider-row{grid-template-columns:1fr}.provider-row>strong{grid-column:auto}}
