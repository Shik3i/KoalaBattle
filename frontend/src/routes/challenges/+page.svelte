<script lang="ts">
  ...
  // The endpoint pages; the list used to request one default page of 100 and show it
  // as if it were everything, so a longer history silently lost its older runs.
  const HISTORY_PAGE_SIZE = 25;

  async function loadHistory() {
    historyLoading = true;
    try {
      runs = await api<ChallengeRunSummary[]>(`/api/challenges?limit=${HISTORY_PAGE_SIZE}`);
      hasMoreHistory = runs.length === HISTORY_PAGE_SIZE;
      historyError = '';
    } catch (caught) {
  ...
  async function loadMoreHistory() {
    ...
    try {
      const next = await api<ChallengeRunSummary[]>(
        `/api/challenges?limit=${HISTORY_PAGE_SIZE}&offset=${runs.length}`
      );
      // Guard against a run being created between pages, which would otherwise
      // shift the offset window and repeat an entry.
      const known = new Set(runs.map((run) => run.id));
      runs = [...runs, ...next.filter((run) => !known.has(run.id))];
      hasMoreHistory = next.length === HISTORY_PAGE_SIZE;
      historyError = '';
    } catch (caught) {
      historyError = caught instanceof Error ? caught.message : String(caught);
    } finally {
  ...
  {:else}
    ...
    {#if hasMoreHistory}
      <div class="history-more">
        <button class="button secondary" type="button" disabled={historyLoadingMore} on:click={loadMoreHistory}>
          <i class={`ph ${historyLoadingMore ? 'ph-spinner-gap spinner' : 'ph-caret-down'}`} aria-hidden="true"></i>
          {historyLoadingMore ? 'Loading…' : 'Show older runs'}
        </button>
      </div>
    {/if}
  {/if}
...
<style>
  .challenge-head{margin-bottom:1rem}.challenge-head>div:first-child{max-width:720px}.challenge-actions{display:flex;flex-wrap:wrap;justify-content:flex-end;gap:.5rem}.quick-start{min-width:13.5rem}.quick-start>span{display:grid;justify-items:start;line-height:1.15}.quick-start strong{font-size:.78rem}.quick-start small{font:500 0.72rem/1.25 var(--mono);opacity:.75}.quick-start-error{margin:-.45rem 0 1rem;padding:.65rem .8rem;border:1px solid color-mix(in srgb,var(--danger) 55%,var(--border));border-radius:.6rem;background:color-mix(in srgb,var(--danger) 8%,var(--panel));color:var(--danger);font-size:.75rem}.spinner{animation:spin .8s linear infinite}.challenge-head p,.panel p,.empty p{color:var(--muted);line-height:1.55}.mode-intro{display:grid;grid-template-columns:1.05fr 1.6fr;gap:1.25rem;padding:1.4rem;border-color:color-mix(in srgb,var(--accent) 35%,var(--border));background:linear-gradient(135deg,color-mix(in srgb,var(--accent) 7%,var(--panel)),var(--panel))}.intro-copy h2{margin:.3rem 0;font-size:1.55rem}.intro-copy p{margin-bottom:0}.intro-facts{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.55rem}.intro-facts>span{display:grid;grid-template-columns:auto 1fr;gap:.12rem .55rem;padding:.7rem;border:1px solid var(--border);border-radius:.65rem;background:var(--panel-strong)}.intro-facts i{grid-row:1/3;align-self:center;color:var(--accent);font-size:1.25rem}.intro-facts strong{font-size:.78rem}.intro-facts small{color:var(--muted);font:0.72rem/1.35 var(--mono)}.mode-intro>details{grid-column:1/-1}.mode-intro ol{margin:.7rem 0 0;padding-left:1.2rem;color:var(--muted);font-size:.78rem;line-height:1.65}.campaign-library{margin-top:1.5rem}.campaign-library>header{display:flex;align-items:end;justify-content:space-between;margin-bottom:.65rem}.campaign-library h2{margin:.25rem 0}.campaign-library>header>span{color:var(--muted);font:0.72rem var(--mono)}.campaign-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:.65rem}.campaign-card{display:grid;gap:.25rem;padding:1rem;box-shadow:none;transition:transform .16s ease,border-color .16s ease}.campaign-card:hover,.campaign-card:focus-visible{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 45%,var(--border))}.campaign-card-head{display:flex;align-items:center;justify-content:space-between;color:var(--accent)}.campaign-card-head i{font-size:1.2rem}.generation-mark{font:700 0.72rem var(--mono);letter-spacing:.08em}.campaign-card h3{margin:.35rem 0 0;font-size:1rem}.campaign-card p{margin:0;font:0.72rem var(--mono)}.campaign-card small{color:var(--muted);font-size:0.72rem}.history{margin-top:1.5rem}.history-more{display:flex;justify-content:center;margin-top:.8rem}.history>header{display:flex;align-items:end;justify-content:space-between;margin-bottom:.65rem}.history h2{margin:.25rem 0}.history>header>span{color:var(--muted);font:0.72rem var(--mono)}.run-list{display:grid;gap:.65rem}.run-list>a{display:grid;grid-template-columns:minmax(0,1fr) auto minmax(180px,auto) auto;align-items:center;gap:1.4rem;padding:1rem 1.2rem;box-shadow:none;transition:transform .16s ease,border-color .16s ease}.run-list>a:hover,.run-list>a:focus-visible{transform:translateY(-2px);border-color:color-mix(in srgb,var(--accent) 45%,var(--border))}.run-name h3{margin:.25rem 0}.run-name small,.run-state small{display:block;color:var(--muted);font:0.72rem var(--mono)}.progress{display:grid;text-align:right}.progress strong{font-size:1.2rem}.progress span{color:var(--muted);font:0.72rem var(--mono)}.run-state{display:grid;justify-items:start;gap:.25rem}.empty{display:grid;place-items:center;padding:3rem;text-align:center}.empty>.ph{color:var(--accent);font-size:2rem}@keyframes spin{to{transform:rotate(360deg)}}@media(max-width:950px){.campaign-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:850px){.page-head{align-items:stretch;flex-direction:column}.challenge-actions{justify-content:flex-start}.mode-intro{grid-template-columns:1fr}.intro-facts{grid-template-columns:1fr 1fr}.run-list>a{grid-template-columns:1fr auto}.run-list>a>.ph{display:none}.run-state{grid-column:1/-1}.progress{text-align:right}}@media(max-width:560px){.challenge-actions>*{flex:1}.intro-facts,.campaign-grid,.run-list>a{grid-template-columns:1fr}.progress{text-align:left}.history>header,.campaign-library>header{align-items:flex-start;flex-direction:column}}@media(prefers-reduced-motion:reduce){.spinner{animation:none}}
</style>
