...
{#if loading}<Skeleton rows={4} label="Loading the match archive…" />{:else if error}<p class="error">{error}</p>{:else if !matches.length}<section class="empty panel"><h2>Nothing in the archive matches this filter</h2><p>Widen the search or clear the status filter — or start a new battle.</p><a class="button" href="/new"><i class="ph ph-plus" aria-hidden="true"></i>New battle</a></section>{:else}<div class="match-list">{#each matches as match}<MatchCard {match} headingLevel={2} />{/each}</div>{/if}
