...
<section class="library"><div class="section-head"><div><span class="eyebrow">Immutable snapshots</span><h2>Saved teams</h2></div><span>{teams.length} local</span></div>
  ...
  <div class="cards">{#each teams as team}<details class="panel team"><summary><div><strong>{team.name}</strong><small>{team.source} · {team.format} · {new Date(team.created_at).toLocaleString()}</small></div><span>✓ Legal</span></summary><div class="team-body"><div class="tabs"><span>Showdown Export</span><span>Normalized Team</span><span>Validation</span>{#if team.generation_audit}<span>Generation Audit</span>{/if}</div><textarea readonly value={team.normalized_export}></textarea><button class="button secondary" on:click={() => copy(team)}>Copy Showdown team</button></div></details>{/each}</div>
</section>

<style>
  .page-head,.section-head{display:flex;justify-content:space-between;align-items:end;gap:2rem}.page-head h1{font-size:clamp(2.2rem,6vw,4.6rem);margin:.3rem 0}.page-head p,.builder p{color:var(--muted);max-width:680px}.team-workspace{display:grid;grid-template-columns:1.4fr .8fr;gap:1rem;margin-top:2rem}.editor,.builder{padding:1.3rem;display:grid;gap:1rem;align-content:start}.editor textarea{min-height:390px;font:400 .72rem/1.5 var(--mono)}.builder h2{margin:.2rem 0}.result{padding:.8rem;border:1px solid var(--danger);border-radius:10px;color:var(--danger)}.result.valid{border-color:var(--accent);color:var(--accent)}.result p{margin:.35rem 0;font:.7rem var(--mono)}.library{margin-top:3rem}.cards{display:grid;gap:.7rem;margin-top:1rem}.team{box-shadow:none}.team summary{display:flex;justify-content:space-between;align-items:center;padding:1rem;cursor:pointer}.team summary div{display:grid}.team small{color:var(--muted);font:.67rem var(--mono)}.team summary>span{color:var(--accent);font:.68rem var(--mono)}.team-body{padding:0 1rem 1rem;border-top:1px solid var(--border)}.team-body textarea{width:100%;min-height:300px;font:400 .7rem/1.5 var(--mono)}.tabs{display:flex;gap:1rem;padding:.8rem 0;color:var(--muted);font:.65rem var(--mono);overflow:auto}.empty{padding:2rem;color:var(--muted)}@media(max-width:850px){.page-head{align-items:start;flex-direction:column}.team-workspace{grid-template-columns:1fr}}
</style>
